...
import { getTopCategories } from "../../api/productSales";

function AllTopCategories(props) {
  const { setAllCategories, filterState, months, warehouses, selectingDate, currencyFormat } = props;
  ...
  const RenderData = () => {
    return salesList?.map((sale, index) => {
      return (
        ...
        >
          <h1 className="col-span-3 font-bold">{sale.category_name}</h1>
          <h1 className="col-span-2 text-center">{sale.totalItem} item(s)</h1>
          <h1 className="text-right col-span-4">{currencyFormat(parseInt(sale.totalTransaction))}</h1>
        </div>
      );
    });
  };
  ...
  return (
    <div className="modal-background">
      <div className="modal-container h-fit w-[93%]">
        <ClosedBtnModal setModal={setAllCategories} />
        <div>
          <h1 className="modal-header-text">Top Sales By Categories</h1>
          <div
          ...
          >
            ...
            >
              <h1 className="col-span-3 ">category</h1>
              <h1 className="col-span-2 text-center">qty</h1>
              <h1 className="text-right col-span-4">total sales</h1>
            </div>
            <RenderData />
          </div>
        </div>
...
export default AllTopCategories;
